...
export function useAdminUsers() {
    ...
    const { data: users, pending, refresh } = useLazyAsyncData(
        "admin-users",
        () => $fetch("/api/users"),
    );
    ...
    const filteredUsers = computed(() => {
        ...
        return (users.value || []).filter((u: any) =>
            ...
            u.id?.toLowerCase().includes(q) || u.phone?.includes(q)
        );
    });
    ...
    const stats = computed(() => {
        ...
        const active = list.filter((u: any) => {
            ...
            return new Date(u.last_sign_in_at).toDateString() ===
    ...
    const editSchema = z.object({
        ...
        password: z.string().min(6, "6 caractères minimum").optional().or(
            z.literal(""),
        ),
    ...
    async function onEditSubmit(event: { data: EditSchema }) {
        ...
        const body: any = {};
        if (event.data.email && event.data.email !== selectedUser.value.email) {
            body.email = event.data.email;
        }
        if (event.data.password) body.password = event.data.password;
        if (event.data.phone && event.data.phone !== selectedUser.value.phone) {
            body.phone = event.data.phone;
        }
        body.email_confirm = event.data.email_confirm !== false;
        if (
            !body.email && !body.password && !body.phone &&
            (!event.data.email_confirm ||
                body.email_confirm === !!selectedUser.value.email_confirmed_at)
        ) {
            toast.add({
                title: "Info",
    ...
    async function onInviteSubmit(event: { data: InviteSchema }) {
        ...
        try {
            ...
            toast.add({
                title: "Succès",
                description: `Invitation envoyée à ${event.data.email}.`,
                color: "success",
            });
    ...
    async function generateRecoveryLink(user: any) {
        ...
        try {
            ...
            toast.add({
                title: "Lien copié",
                description: "Le lien de réinitialisation a été copié.",
                color: "success",
            });
        } catch (err: any) {
            toast.add({
    ...
    async function openDetailModal(user: any) {
        ...
        try {
            ...
            try {
                detailFactors.value = await $fetch(
                    `/api/users/${user.id}/factors`,
                );
            } catch {
                detailFactors.value = [];
            }
