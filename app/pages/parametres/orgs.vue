...
const { data: classes, refresh } = await useFetch<Organisation[]>('http://127.0.0.1:4000/classes', { default: () => [] })
...
<template>
    <UDashboardPage>
            ...
            <UDashboardNavbar title="Organisations" :badge="filteredOrganisations.length">
                <template #right>
                    <UButton icon="i-heroicons-plus-16-solid" color="teal" variant="ghost"
                        @click="isNewOrganisationModalOpen = true" />
                </template>
            </UDashboardNavbar>
                    ...
                    <template #left>
                        <UTooltip text="Move to junk">
                            <UButton icon="i-heroicons-arrow-path" color="gray" variant="ghost"
                                @click="fetchLookups(selectedOrganisation.id)" />
                        </UTooltip>
                    </template>
        ...
        <UDashboardModal v-model="isNewLookupsModalOpen" :title="`${actionToSubmit} un Lookups`"
            ...
            <OrganisationLookupsForm :classe="selectedOrganisation" :action='actionToSubmit' :lookups="selectedLookups"
                @close="modalLookupsClosed" />
        </UDashboardModal>
        <UDashboardModal v-model="isNewOrganisationModalOpen" title="Ajouter une classe" description="Ajouter une classe"
            :ui="{ width: 'sm:max-w-md' }">
            <!-- ~/components/users/UsersForm.vue -->
            <OrganisationsForm @close="modalOrganisationClosed" />
        </UDashboardModal>
    </UDashboardPage>
</template>
